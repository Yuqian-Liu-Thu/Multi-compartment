...
from MCSRNN_PRCargs import PRCPARS
...
class NetworkArchitecture:
    nb_units_by_layer: Sequence[int] = (100, 4, 2) #random pattern task
    #nb_units_by_layer: Sequence[int] = (80, 100, 10) #smnist pattern task
...
def _zeros(shape):
    """Get zeros tensor with device and dtype determined by PRCPARS."""
    return torch.zeros(
        shape, device=PRCPARS.device, dtype=PRCPARS.dtype
    )


def _time_constant_to_discount_factor(time_constant: float) -> float:
    return float(np.exp(-PRCPARS.time_step / time_constant))
...
class Synapse(Subunit):
    def __init__(self, nb_units: int, time_constant: float):
        initial_state = _zeros((PRCPARS.batch_size, nb_units))
        super().__init__(
            time_constant,
            initial_state,
            initial_state,
...
class Neuron:
    ...
    def __init__(self, parameters: NeuronParameters, nb_units: int):
        ...
        self.somatic_subunit = Subunit(
            parameters.tau_mem,
            _zeros((PRCPARS.batch_size, nb_units)),
            _zeros((PRCPARS.batch_size, nb_units)),
...
class SpikingNetwork:
    ...
    def _run_layer(input_: torch.Tensor, units: Neuron) -> Recorder[Neuron]:
        ...
        for t in range(PRCPARS.nb_steps):
